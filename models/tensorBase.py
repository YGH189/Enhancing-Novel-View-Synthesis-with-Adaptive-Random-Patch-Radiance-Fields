...
class LearnedPositionalEncoding(torch.nn.Module):
    def __init__(self, input_dim, num_freqs):
        ...
        self.phase = torch.nn.Parameter(torch.randn(input_dim, num_freqs))
    ...
    def forward(self, x):
        """
        :param x: (..., input_dim)
        :return: (..., input_dim * num_freqs * 2)
        """
        shape = x.shape
        x = x.view(-1, self.input_dim)  # [N, input_dim]


        x_freq = x[..., None] * self.freq_bands.to(x.device) + self.phase.to(x.device)  # [N, input_dim, num_freqs]


        x_encoded = torch.cat([torch.sin(x_freq), torch.cos(x_freq)], dim=-1)  # [N, input_dim, num_freqs * 2]


        x_encoded = x_encoded.view(shape[0], -1)

        return x_encoded.view(*shape[:-1], -1)
...
class MLPRender_Fea(torch.nn.Module):
    def __init__(self, inChanel, viewpe=6, feape=6, featureC=128):
        ...
        self.pos_encoding = LearnedPositionalEncoding(input_dim=inChanel, num_freqs=feape)  # features的频带数量
    ...
    def forward(self, pts, viewdirs, features):
        ...
        if self.feape > 0:
            indata += [self.pos_encoding(features)]
        if self.viewpe > 0:
...
class MLPRender_PE(torch.nn.Module):
    def __init__(self, inChanel, viewpe=6, pospe=6, featureC=128):
        ...
        self.pos_encoding = LearnedPositionalEncoding(input_dim=3, num_freqs=pospe)
